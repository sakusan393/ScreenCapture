...
- Windows の表示言語が日本語の場合は対象メニューとダイアログが日本語、英語の場合は英語で表示される。
- 右クリックからテキストを追加し、編集、移動、色、背景色、サイズ、削除を操作できる。
- 最後に変更した文字色、背景色、文字サイズが、再起動後に追加したテキストへ反映される。
- `Ctrl+V` でクリップボード画像と、エクスプローラーでコピーした JPG / JPEG / PNG ファイルを追加できる。
- 透過 PNG の透明度が維持され、キャプチャ領域より大きい画像は縦横比を保って領域内へ自動縮小される。
- 複数の画像ファイルを貼り付けると、すべての画像が少しずつずれて追加される。
- 貼り付け画像を移動し、四隅リサイズ、Shift 比率固定、回転、枠、削除を操作できる。
- Layers パネルの並べ替えが画面の前後関係へ反映され、再起動後も残る。
- 前面へ移動した画像／テキストが同じグループ内の順序だけを変更する。
- 左／上／右／下へ領域を広げても、元画像と注釈の見た目の相対位置が保たれる。
- キャプチャ背景色で Alpha 0 を選択しても 1 へ補正され、透明な余白からウィンドウをドラッグできる。
