...
`AllowsTransparency` が有効なウィンドウでは完全透明な背景部分が OS のヒットテスト対象外になるため、キャプチャ背景色に限り Alpha 0 を 1 へ正規化します。カラーピッカーからの変更と既存設定の読み込みの両方に適用し、テキスト背景など完全透明を必要とする他の色設定には適用しません。
...
`CaptureWindow` の `Ctrl+V` は、クリップボード上の画像データを優先し、画像がない場合は `FileDrop` の JPG / JPEG / PNG ファイルを順番に読み込みます。ファイルは `BitmapCacheOption.OnLoad` 相当で完全に読み込んでからストリームを閉じるため、貼り付け後に元ファイルをロックしません。

PNG のクリップボード形式がある場合はエンコード済みデータを直接デコードして透明度を維持します。DIB / DIBV5 は WPF が未使用のアルファ値を完全透明と解釈する場合があるため、`System.Drawing` 経由の変換を使用します。

貼り付け画像は元のピクセル寸法を基準にし、配置位置からキャプチャ領域の右端・下端までに収まらない場合だけ、縦横比を維持して縮小します。小さい画像は拡大しません。複数ファイルは少しずつ位置をずらして配置します。
